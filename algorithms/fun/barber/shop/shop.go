...
type Shop struct {
	...
	chairs  chan customer  // The set of chairs in the shop.   //跟信号量有关的内容go里面都是用channel实现的.
	wgClose sync.WaitGroup // Provides support for closing the shop.   //闭店必须客人都消费完才行,所以需要一个watigroup
	wgEnter sync.WaitGroup // Tracks customers entering the shop.  //
}

// Open creates a new shop for business and gets the barber working. 就是New函数,表示开始营业,创造shop实体. 为了效率这里面返回指针.
func Open(maxChairs int) *Shop {
	s := Shop{
		chairs: make(chan customer, maxChairs),
	}
	atomic.StoreInt32(&s.open, 1)  //感觉这个地方不涉及并发, 所以应该可以改成直接赋值.

	// Get the barber working.
	s.wgClose.Add(1)
	go func() {
		defer s.wgClose.Done()
		for cust := range s.chairs { //对于每个椅子上面的顾客进行循环即可.
			fmt.Printf("Barber servicing customer %q\n", cust.name)
			time.Sleep(time.Duration(rand.Intn(500)) * time.Millisecond) //模拟服务时长
