...
func (h *Hash) Store(key string, value int) {
	...
	idx := h.hashKey(key) //对key进行哈希算法.得到一个index
...
//下面是一个哈希算法算哈希的流程,先reset再写进去.再读取
...
func removeEntry(bucket []entry, idx int) []entry {
	...
	// entries removed from this bucket. //因为每次减,如果减得次数过多,会发生capacity过大,所以进行capacity压缩.
	return reduceAllocation(bucket)
}

// reduceAllocation looks to see if memory can be freed to
// when a bucket has lost a percent of entries. //优化空间, 节省capacity的容量.
